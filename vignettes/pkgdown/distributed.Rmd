...
Distributed LDA can dramatically speeds up your analysis by using multiple processors on your computer. The corpus and the number of topic is small in this example, but the distributed algorithm is highly effective in identifying many topics (`k > 100`) in a large corpus.
...
The algorithm fits LDA through 2000 iterations by default to produce reliable results, but we can can minimize the number using the convergence detection mechanism. When `auto_iter = TRUE`, the algorithm stop inference on convergence (`delta < 0`) and return the results to further speed up your analysis.
