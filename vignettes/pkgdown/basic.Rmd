...
**seededlda** was created mainly for semi-supervised topic modeling but it can perform unsupervised topic modeling too. I explain the basic functions of the package taking unsupervised topic modeling as an example in this page and discuss semi-supervised topic modeling in [a separate page](seeded.html).
...
You can fit LDA on the DFM only by setting the number of topics `k = 10` to identify. When `verbose = TRUE`, it shows the progress of the inference through iterations. It takes long to fit LDA on a large corpus, but [distributed algorithm](distributed.html) will speed up your analysis dramatically.
