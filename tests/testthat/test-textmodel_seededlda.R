...
test_that("levels is working", {
  ...
  set.seed(1234)
  lda1 <- textmodel_seededlda(dfmt, dict, levels = 1, residual = TRUE, weight = 0.02,
                              min_termfreq = 10, max_iter = 100)
  expect_equal(
      levels(topics(lda1)),
      c("romance", "sifi", "other")
  )
  lda2 <- textmodel_seededlda(dfmt, dict, levels = 1:2, residual = TRUE, weight = 0.02,
                              min_termfreq = 10, max_iter = 100)
  expect_equal(
      levels(topics(lda2)),
      c("romance", "sifi.space", "sifi.monster", "other")
  )
  lda3 <- textmodel_seededlda(dfmt, dict, levels = 2, residual = TRUE, weight = 0.02,
                              min_termfreq = 10, max_iter = 100)
  expect_equal(
      levels(topics(lda3)),
  ...
  expect_error(
      textmodel_seededlda(dfmt, dict, levels = -1, residual = TRUE, weight = 0.02,
                          min_termfreq = 10, max_iter = 100),
